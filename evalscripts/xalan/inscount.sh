...
# -taint argument points to theInputSource arg of transform function (which is passed as first arg to the parseSource function afterwards and is read by instruction at the 3284cf static address) until sizeof(XSLTInputSource) bytes forward
args="-pin_memory_range 0x4000000000:0x6000000000 -t ./obj-intel64/SE.so -trace tmp/se/trace.dat -memory tmp/se/memory.dat -measure -endpoints 0x7ffff60d8040,0x7ffff60d80b3 -taint 0x7fffffffdd00,0x7fffffffdd40 -verbose $verbose -logfilename out-eval-$progname -- $progexec"
