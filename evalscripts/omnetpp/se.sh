...
#     cat out-eval-omnetpp.log | grep -A2 7ffff619dc1e | grep Expression | gawk -F'Expression\\(' '{ print $2 }' | gawk -F'\\)' '{ print $1 }' | sed 's/\x1B\[[0-9;]*[JKmsu]//g' | xargs echo
args="-pin_memory_range 0x4000000000:0x6000000000 -t $CUR_FOLDER/obj-intel64/SE.so -trace $CUR_FOLDER/tmp/se/trace.dat -memory $CUR_FOLDER/tmp/se/memory.dat -endpoints 0x7ffff619d96b,0x7ffff619dd81 -symbols 0x6d1cb0,0x6d1cf8,0x6cfe10,0x6cfe58,0x6c5700,0x6c5748 -verbose $verbose -logfilename $CUR_FOLDER/out-eval-$progname -- $progexec"
